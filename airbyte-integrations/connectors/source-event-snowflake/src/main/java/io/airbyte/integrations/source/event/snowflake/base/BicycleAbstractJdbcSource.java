...
package io.airbyte.integrations.source.event.snowflake.base;
...
import io.airbyte.integrations.source.event.bigquery.BicycleAbstractRelationalDbSource;
 ...
 */
public abstract class BicycleAbstractJdbcSource<Datatype>
        extends BicycleAbstractRelationalDbSource<Datatype, JdbcDatabase>
        implements Source {

    private static final Logger LOGGER = LoggerFactory.getLogger(BicycleAbstractJdbcSource.class);
    ...
    public BicycleAbstractJdbcSource(final String driverClass,
    ...
    private String getCatalog(final SqlDatabase database) {
        return (database.getSourceConfig().has("database") ? database.getSourceConfig().get("database").asText() :
                null);
    }

    @Override
    protected List<TableInfo<CommonField<Datatype>>> discoverInternal(final JdbcDatabase database, final String schema)
            throws Exception {
     ...
     *              {@link BicycleAbstractJdbcSource#getColumnMetadata}.
     */
            ...
            final Map<String, List<String>> tablePrimaryKeys = aggregatePrimateKeys(database.bufferedResultSetQuery(
                    ...
                    r -> {
                        final String schemaName =
                                r.getObject(JDBC_COLUMN_SCHEMA_NAME) != null ? r.getString(JDBC_COLUMN_SCHEMA_NAME) :
                                        r.getString(JDBC_COLUMN_DATABASE_NAME);
                        final String streamName = sourceOperations.getFullyQualifiedTableName(schemaName,
                                        ...
                                        aggregatePrimateKeys(database.bufferedResultSetQuery(
                                                conn -> conn.getMetaData()
                                                        .getPrimaryKeys(getCatalog(database), tableInfo.getNameSpace(),
                                                                tableInfo.getName()),
                                                r -> new SimpleImmutableEntry<>(streamName,
            ...
            try {
                final Stream<JsonNode> stream = database.unsafeQuery(
                        connection -> {
                            ...
                            String identifier = sourceOperations.enquoteIdentifier(connection, cursorField);
                            final String sql =
                                    String.format("SELECT %s FROM %s WHERE %s > ? ORDER BY %s limit %d",
                                            sourceOperations.enquoteIdentifierList(connection, columnNames),
                                            sourceOperations
                                                    .getFullyQualifiedTableNameWithQuoting(connection, schemaName,
                                                            tableName),
                                            identifier,
                                            identifier, bigQueryEventSourceConfig.getDefaultLimit());

                            final PreparedStatement preparedStatement = connection.prepareStatement(sql);
                            String modifiedCursor = cursor;
                            //This is done as there are cases when cursor field is returned as scientific notation
                            //and when it's parsed as long, parsing fails.
                            try {
                                if (cursorFieldType.equals(JDBCType.BIGINT)) {
                                    double doubleValue = Double.parseDouble(cursor);
                                    long result = (long) doubleValue;
                                    modifiedCursor = String.valueOf(result);
                                }
                            } catch (Exception e) {

                            }
                            sourceOperations.setStatementField(preparedStatement, 1, cursorFieldType, modifiedCursor);
